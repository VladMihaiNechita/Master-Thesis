...
from ...MAE.fine_tuning.data_transforms import (IMAGENET_MEAN, IMAGENET_STD, BatchMixupCutmix, DeiTRandomErasing, build_training_transform)
from ....models import IJEPAFineTuningModel
...
def build_optimizer_parameter_groups(model, layer_decay):
    ...
    for name, parameter in model.named_parameters():
        ...
        weight_decay = 0.05 if parameter.ndim > 1 else 0.0
...
def main(fine_tuning_config, fine_tuning_checkpoint_path=None,
    ...
    model = IJEPAFineTuningModel(config, num_classes=len(train_dataset.classes))

    if fine_tuning_checkpoint_path is not None:
        model.load_state_dict(checkpoint["model"])
    else:
        # The paper evaluates the EMA target encoder and discards the predictor.
        model.encoder.load_state_dict(checkpoint["target_encoder"], strict=True)
        if not pretraining_checkpoint_path.stem.endswith("images_seen_0"):
            # Synthetic pretraining used raw [0, 1] images. Adapt only once, when loading
